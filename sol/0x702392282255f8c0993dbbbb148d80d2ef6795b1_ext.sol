...
/*
    Yes, both functions transfer() and withdraw() can lead to reentrancy bugs.

In the transfer() function, the withdraw() function is called if the myDividends(true) 
function returns a value greater than zero. If the withdraw() function has not completed 
its execution and it is called again from the transfer() function, then it can cause a 
reentrancy attack.
