...
/*
    In the withdraw() function, there is a call to _customerAddress.transfer(_dividends), which is an external 
    call to transfer ether to _customerAddress. If the external contract being called in transfer() has a 
    fallback function that calls back into the withdraw() function before the previous invocation has finished 
    executing, then a reentrancy vulnerability exists.

In the transfer() function, there is a call to withdraw() function if myDividends(true) > 0. If withdraw() 
function contains a reentrancy vulnerability, it could be exploited by an attacker to continuously withdraw 
dividends during the transfer process.

Therefore, both withdraw() and transfer() functions may have reentrancy vulnerabilities that need to be addressed.
 */
/*
