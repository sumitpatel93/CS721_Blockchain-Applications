...
/* Yes, this contract have a reentrancy bug. In the refunding() function, there is a call to the msg.sender.send() 
   function, which can potentially call an external contract that can then call back into this contract's function,
   re-entering the function and causing unexpected behavior.

   To fix this, the contract should use the transfer() function instead of send(), since transfer() limits the 
   amount of gas that the recipient contract can consume, preventing it from re-entering the current function. 
   */
library SafeMath {
